...
lm_predict <- predict(model_lm , newdata=test )
lm_predict <- lm_predict %>% as_data_frame()
lm_predict <- exp(lm_predict)
prediction1 <- cbind(test$id,lm_predict)
names(prediction1)[1] <- "id"
names(prediction1)[2] <- "price_doc"

#write the document 
write_csv(prediction1,"sumbmission1.csv")

#plot the prediction of test 
hist(x= prediction1$price_doc)
...
#predict the 
lasso_predict <- predict(LASSO_fit, newx=predictor_matrix_test, s=lambda_star_1SE) %>% 
  as.vector()
lasso_predict <- lasso_predict %>% as_data_frame()
lasso_predict <- exp(lasso_predict)  
prediction2 <- cbind(test$id,lasso_predict)
names(prediction2)[1] <- "id"
names(prediction2)[2] <- "price_doc"

 
write_csv(prediction2,  "sumbmission2.csv")

#plot the prediction of test 
hist(x= prediction2$price_doc)
...
--------------------- | -------------   | -------------
lm                    | 0.2534705       |  2.63413
LASSO crossvalidated  | 0.3799357       |  0.42577
